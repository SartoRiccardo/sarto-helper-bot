...
class REditorCog(commands.Cog):
    ...
    async def check_add_video_meta(self, message: discord.Message) -> None:
        ...
        Fired by on_message
        """
        if message.reference is None or \
                not self.is_thumbnail_channel(message.reference.guild_id, message.reference.channel_id):
            return
        has_thumbnail = len(message.attachments) > 0 or "https://" in message.content
        ...
        is_short = "\N{SHORTS}" in title or not has_thumbnail
        title = title.replace("\N{SHORTS}", "").strip()
    ...
    def get_video_list_message(interaction: discord.Interaction,
        ...
        for v in video_list:
            if not v['title']:
                ...
                category = modules.util.discordutils.get_reditor_category(interaction.guild)
                if not category:
